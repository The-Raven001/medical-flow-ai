...
from app.schemas.patients import PatientsSummary
from typing import List

class ClinicUsersBase(BaseModel):
...
class ClinicUsersCreate(ClinicUsersBase):
    password: str = Field(..., min_length=8, example="12345")

class ClinicUsersResponse(ClinicUsersBase):
    id: int
    model_config = ConfigDict(from_attributes=True)

        

class ClinicUsersLogin(BaseModel):
    email: EmailStr = Field(..., example= "useremail@example.com")
    password: str = Field(..., min_length=8, example="12345")
